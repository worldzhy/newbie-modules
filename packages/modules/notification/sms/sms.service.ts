...
import {SmsNotification} from '@generated/prisma/client';
...
export class SmsService {
  ...
  async send(params: {phone: string; text: string}): Promise<SmsNotification> {
    ...
    const output: SendMessagesCommandOutput = await this.pinpointService.sendSms({
    ...
    if (output.MessageResponse) {
      ...
      if (output.MessageResponse?.Result) {
        pinpointMessageId = output.MessageResponse?.Result[params.phone].MessageId;
      }
    }
